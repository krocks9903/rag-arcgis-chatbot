// Administrator console: status, corpus swap, report inbox.
// Prefer window.API_BASE from config.js ("" means same-origin). Do not treat
// empty string as missing — that would force localhost:8000 and break admin
// when the API is served from the same origin as this page.
const API_BASE = (
  typeof window !== "undefined" && typeof window.API_BASE === "string" ? window.API_BASE : window.location.origin
).replace(/\/$/, "");
...
async function refreshStatus() {
  ...
  try {
    ...
    grid.innerHTML = `
      <dt>Index loaded</dt><dd>${data.index_loaded ? "Yes" : "No"}</dd>
      <dt>Board records</dt><dd>${esc(data.record_count)}</dd>
      <dt>Embedding</dt><dd>${esc(data.embedding_model)}</dd>
      <dt>Reranker</dt><dd>${esc(data.reranker_model || "disabled")}</dd>
      <dt>Open reports</dt><dd>${esc(reports.open ?? 0)} / ${esc(reports.total ?? 0)}</dd>
      <dt>Board CSV</dt><dd>${esc(data.board_csv)}</dd>
    `;
...
async function swapCsv() {
  const input = document.getElementById("csv-path-input");
  const statusEl = document.getElementById("load-status");
  const btn = document.getElementById("load-btn");
  const csvPath = (input.value || "").trim();
  if (!csvPath) {
    setStatus(statusEl, "Enter a CSV filename first (e.g. data.csv)", false);
    return;
  }
  btn.disabled = true;
  setStatus(statusEl, "Rebuilding index…");
  try {
    ...
    setStatus(statusEl, e.message || "Rebuild failed", false);
  } finally {
    btn.disabled = false;
  }
}
...
function wire() {
  ...
  document.getElementById("load-btn").addEventListener("click", swapCsv);
