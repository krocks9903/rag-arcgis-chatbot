    ...
    <div className="report-backdrop" role="presentation" onClick={onClose}>
      ...
      >
        <form onSubmit={submit}>
          ...
          <select id="report-kind" value={kind} onChange={(e) => setKind(e.target.value as ReportKind)} required>
          ...
          <input id="report-current" value={currentValue} onChange={(e) => setCurrentValue(e.target.value)} maxLength={1000} />
          <label className="report-label" htmlFor="report-suggested">
            What should it say? (optional)
          </label>
          <input id="report-suggested" value={suggestedValue} onChange={(e) => setSuggestedValue(e.target.value)} maxLength={1000} />
          ...
          {status.text && (
            <p className={`report-form-status${status.ok === true ? " ok" : status.ok === false ? " err" : ""}`}>
              {status.text}
            </p>
          )}
        </form>
